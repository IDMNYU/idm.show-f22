...
let pTitleKey = "Project title";
let pMainURLKey = "Project link";
let pShortDescKey = "Short project description (one sentence - 500 characters)";
let pLongDescKey = "Long Description of your work (4-6 sentences).";
let pHashtagsKey = "Project Hashtag #";
let pKeywordsKey = "Theme and keywords";
let sPersonalURLKey = "Link to your personal portfolio";
let sLinkedinKey = "Linked In Handle";
let sInstagramKey = "Instagram";
let sYoutubeKey = "Link to any video content";
let sTwitterKey = "Twitter";
let pAdditionalMediaKey = "Supplementary Images";
...
function initPage(event) {
  ...
  let colorAString =
    "rgba(" +
    newRandomGradientColorA_R +
    "," +
    newRandomGradientColorA_G +
    "," +
    newRandomGradientColorA_B +
    ")";
  let colorBString =
    "rgba(" +
    newRandomGradientColorB_R +
    "," +
    newRandomGradientColorB_G +
    "," +
    newRandomGradientColorB_B +
    ")";
...
function produceProjectItemPage() {
  ...
  const project = projectCollection.projects.find(
    (proj) => proj[pIDKey] === paramProjectID
  );
  let newProjectItemElement = produceItemContent(project);
...
function produceItemContent(projectJSON) {
  ...
  /* Project Main Image */
  projectMainImageElem.src =
    "projects/" +
    projectJSON[pIDKey] +
    "/main.jpg"; /* OUTPUT: `/projectmedia/95/main.jpg` */
  ...
  if (projectJSON[pAdditionalMediaKey].length > 0) {
    let parsedAdditionalMediaFiles =
      projectJSON[pAdditionalMediaKey].split(", ");
  ...
  if (
    projectJSON[sPersonalURLKey] != "" &&
    projectJSON[sPersonalURLKey] != null
  ) {
  ...
  for (var i = 0; i < projectCollection["projects"].length; i++) {
    ...
    for (var t = 0; t < splitTags.length; t++) {
      ...
      if (newItem != "" && newItem != null) {
        ...
        allTagList.indexOf(newItem) === -1
          ? allTagList.push(newItem)
...
function getRandomMainImageURL() {
  let randomProjectIndex = Math.floor(
    Math.random() * projectCollection["projects"].length
  );
  let randomImageString =
    "projects/" +
    projectCollection["projects"][randomProjectIndex][pIDKey] +
    "/main.jpg";
